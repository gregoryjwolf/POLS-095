...
# set working directory (this will be unique to your computer)
setwd("~/Library/CloudStorage/OneDrive-DrakeUniversity/Documents/R/095") 

# read in anes data (will work as long as "anes.dta" is in your working directory)
# alternatively, can load in with "Import" in the top right pane using "From Stata"
...
rm(anes) # removes the larger anes data frame from the R environment to free up memory space
...
anes.small.cleaned <- anes.small %>% # clean our anes.small subset of data
  ...
  # ideology (ordinal)
  mutate(ideology = as.factor(V201200)) %>%                 # create variable "ideology" that is an ordered factor version of V201200
  mutate(ideology = recode_factor(.x = ideology,             # note the . before x, which you need when recoding factors
                                  ...
                                  '-8' = NA_character_,      # value of -8 = NA (missing) (a different way to code NA than from above)
                                  '-9' = NA_character_,      # value of -9 = NA (missing)
                                  '99' = NA_character_)) %>% # value of 99 = NA (missing) 
  mutate(ideology = as.ordered(ideology)) %>% # makes sure ideology is an ordered factor
  # trust  
  mutate(trust.experts = ifelse(V202306< 0, NA, V202306)) %>%  # a different way to makes values less than zero NA
  mutate(trust.experts = as.factor(trust.experts)) %>%
  ...
  mutate(trust.experts = as.ordered(trust.experts)) %>% # makes sure trust.experts is an ordered factor
  # age
  mutate(age = as.numeric(V201507x)) %>%             # create age variable as a numeric class
  mutate(age = na_if(x = age, y = -9)) %>%           # change -9 to NA
  # Biden feeling thermometer
  mutate(biden.ft = as.numeric(V202143)) %>%         # create biden.ft variable as a numeric
  mutate(biden.ft = replace(biden.ft, biden.ft<0, NA)) %>%   # replace all values less than 0 = NA
  mutate(biden.ft = replace(biden.ft, biden.ft>100, NA)) %>%
  # Biden affect
  mutate(biden.affect = as.numeric(V202435)) %>%     # create biden.affect variable as a numeric
  mutate(biden.affect = ifelse(biden.affect < 0, NA, biden.affect)) %>%    # replace all values less than 0 = NA
  droplevels() %>%      # drop all variable value labels that are no longer used
...
class(anes.small.cleaned$turnout)        # should be factor
class(anes.small.cleaned$marital.status) # should be factor
class(anes.small.cleaned$ideology)       # should be ordered factor
class(anes.small.cleaned$trust.experts)  # should be ordered factor
class(anes.small.cleaned$age)            # should be numeric
class(anes.small.cleaned$biden.ft)       # should be numeric
class(anes.small.cleaned$biden.affect)   # should be numeric
...
# add variable labels to print in table because "Trust in Experts" makes
# a lot more sense when presenting data than "trust.experts" does.
...
print(x = anes.table, varLabels = TRUE)   # will show number of cases instead of percents
...
# if the p-value is less than 0.05 then we find a statistically
# significant relationship. (If you see "e-13" or any other number,
# then it's going to be less than 0.05.)
